...
- (IBAction)testFat:(id)sender {
    ...
    [delegate.bleMgr sendSENSSUNFATTestFatWithSex:0 userID:1 age:31 heightCM:heightCM heightInch:heightInch * 10 peripheral:peripheral];
}
...
- (void)peripheralDidWrite:(CBPeripheral *)peripheral data:(SSBLEReadWriteData *)data {
    dispatch_async(dispatch_get_main_queue(), ^(void){
        if (data.dataType == 0x32) {
            ...
            [alert show];
        } else if (data.dataType == 0x5A) {
            self.lblTestFat.text = @"";
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"执行清除数据命令成功！" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alert show];
        }
...
- (IBAction)getHistory:(id)sender {
    ...
    self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, 50000);
